Shader "Unlit/JointBilateralBlur"
{
	Properties
	{
	...
	SubShader
	{
				...
				Pass
		{
			...
			sampler2D _BlurPass1;
			float4 _BlurPass1_ST;
			int _size;
			float _sigma;
			float _bluramount;
			...
			v2f vert (appdata v)
			{
				v2f o;
				o.vertex = UnityObjectToClipPos(v.vertex);
				o.uv = TRANSFORM_TEX(v.uv, _BlurPass1);
				return o;
			}
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				for (int ity = -size; ity < size + 1; ity++) {
					...
					//float pdf = sigma;
					float pdf = 0.39894*exp(-0.5*dis / (sigma*sigma)) / sigma;
					bgcolor += pdf * tex2D(_BlurPass1, float2(i.uv.x, i.uv.y+ity*bluramount));
					//bgcolor += pdf1*pdf2 * tex2Dproj(_ColorTexture, UNITY_PROJ_COORD(float4(i.grabPos.x + itx*bluramount, i.grabPos.y + ity*bluramount, i.grabPos.z, i.grabPos.w)));
				}
				return bgcolor;
			}
